...
//
// ============================================================================
package org.talend.components.table.avro;
...
public class DelimitedStringSchemaInferrer {
    ...
    /**
     * Creates Runtime schema from incoming data with the help of Design schema
     * If Design schema doesn't contain dynamic field, then Runtime schema
     * equals to Design schema. If Design schema contains dynamic field, than
     * Runtime schema is creating in following way: <br>
     ...
     * 3. Field names are {@code "dynamic<Index>"} <br>
     ...
     */
    public Schema inferSchema(Schema designSchema, String delimitedString) {
        if (AvroUtils.isIncludeAllFields(designSchema)) {
            return inferDynamicSchema(delimitedString);
        } else {
            return designSchema;
        }
    }
    ...
    /**
     * Creates Runtime schema for dynamic case
     * 
     * @param delimitedString
     *            a line, which was read from file source
     * @return Runtime schema
     */
    private Schema inferDynamicSchema(String delimitedString) {
