 ...
 *
 * This class was modified to add and use the {@link #jobInstance()} method.
 *
 * A {@code BoundedSource} for reading files resident in a Hadoop filesystem (HDFS) using a Hadoop file-based input
 * format.
 *
 * <p>
 ...
 * {
 *     &#064;code
 *     HDFSFileSource&lt;K, V&gt; source = HDFSFileSource.from(path, MyInputFormat.class, MyKey.class, MyValue.class);
 ...
 * {
 *     &#064;code
 *     PCollection&lt;KV&lt;MyKey, MyValue&gt;&gt; records = HDFSFileSource.readFrom(path, MyInputFormat.class, MyKey.class, MyValue.class);
 * }
 * </pre>
 *
 * <p>
 * Implementation note: Since Hadoop's {@link org.apache.hadoop.mapreduce.lib.input.FileInputFormat} determines the
...
public class ConfigurableHDFSFileSource<K, V> extends BoundedSource<KV<K, V>> {
    ...
    /**
     * Creates a {@code HDFSFileSource} that reads from the given file name or pattern ("glob") using the given Hadoop
     * {@link org.apache.hadoop.mapreduce.lib.input.FileInputFormat}, with key-value types specified by the given key
    ...
    public BoundedReader<KV<K, V>> createReader(PipelineOptions options) throws IOException {
        ...
        if (serializableSplit == null) {
            ...
            return new HDFSFileReader<>(this, filepattern, formatClass, serializableSplit.getSplit());
        }
    }
    ...
    public long getEstimatedSizeBytes(PipelineOptions options) {
        ...
        try {
            ...
            Job job = jobInstance(); // new instance
    ...
    /**
     * A wrapper to allow Hadoop {@link org.apache.hadoop.mapreduce.InputSplit}s to be serialized using Java's standard
     * serialization mechanisms. Note that the InputSplit has to be Writable (which most are).
     */
