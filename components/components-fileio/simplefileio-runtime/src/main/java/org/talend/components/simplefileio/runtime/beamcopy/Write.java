...
import org.talend.components.simplefileio.runtime.beamcopy.Sink.WriteOperation;
import org.talend.components.simplefileio.runtime.beamcopy.Sink.Writer;
...
public class Write<T> extends PTransform<PCollection<T>, PDone> {
    ...
    /**
     * Creates a {@link Write} transform that writes to the given {@link Sink}, letting the runner control how many
     * different shards are produced.
     */
    ...
    private Write(Sink<T> sink, @Nullable PTransform<PCollection<T>, PCollectionView<Integer>> computeNumShards,
            @Nullable ValueProvider<Integer> numShardsProvider, boolean windowedWrites) {
        this.sink = sink;
    ...
    public PDone expand(PCollection<T> input) {
        checkArgument(IsBounded.BOUNDED == input.isBounded() || windowedWrites,
                "%s can only be applied to an unbounded PCollection if doing windowed writes", Write.class.getSimpleName());
        this.sink.validate(null);
    ...
    public void populateDisplayData(DisplayData.Builder builder) {
        ...
        builder.add(DisplayData.item("sink", sink.getClass()).withLabel("Write Sink")).include("sink", sink);
        if (getSharding() != null) {
            builder.include("sharding", getSharding());
        } else if (getNumShards() != null) {
            String numShards = getNumShards().isAccessible() ? getNumShards().get().toString() : getNumShards().toString();
            builder.add(DisplayData.item("numShards", numShards).withLabel("Fixed Number of Shards"));
        }
    }
    ...
    /**
     * Gets the {@link PTransform} that will be used to determine sharding. This can be either a static number of shards
     * (as following a call to {@link #withNumShards(int)}), dynamic (by {@link #withSharding(PTransform)}), or
     * runner-determined (by {@link #withRunnerDeterminedSharding()}.
     */
    @Nullable
    ...
    /**
     * Returns a new {@link Write} that will write to the current {@link Sink} using the specified number of shards.
     *
     * <p>
    ...
    /**
     * Returns a new {@link Write} that will write to the current {@link Sink} using the {@link ValueProvider} specified
     * number of shards.
     *
     * <p>
     * This option should be used sparingly as it can hurt performance. See {@link Write} for more information.
    ...
    /**
     * Returns a new {@link Write} that will write to the current {@link Sink} using the specified {@link PTransform} to
    ...
    public Write<T> withSharding(PTransform<PCollection<T>, PCollectionView<Integer>> sharding) {
        checkNotNull(sharding, "Cannot provide null sharding. Use withRunnerDeterminedSharding() instead");
        return new Write<>(sink, sharding, null, windowedWrites);
    }

    /**
     * Returns a new {@link Write} that will write to the current {@link Sink} with runner-determined sharding.
     */
     ...
     *
     * <p>
     * If this option is not specified, windowing and triggering are replaced by {@link GlobalWindows} and
     * {@link DefaultTrigger}.
     *
     * <p>
     * If there is no data for a window, no output shards will be generated for that window. If a window triggers
     * multiple times, then more than a single output shard might be generated multiple times; it's up to the sink
     * implementation to keep these output shards unique.
     *
     * <p>
    ...
    /**
     * Writes all the elements in a bundle using a {@link Writer} produced by the {@link WriteOperation} associated with
     * the {@link Sink}.
     */
    private class WriteBundles<WriteT> extends DoFn<T, WriteT> {
        ...
        public void processElement(ProcessContext c, BoundedWindow window) throws Exception {
            ...
            if (writer == null) {
                ...
                if (windowedWrites) {
                    writer.openWindowed(UUID.randomUUID().toString(), window, c.pane(), UNKNOWN_SHARDNUM, UNKNOWN_NUMSHARDS);
                } else {
    ...
    /**
     * Like {@link WriteBundles}, but where the elements for each shard have been collected into a single iterable.
     *
     * @see WriteBundles
     */
    private class WriteShardedBundles<WriteT> extends DoFn<KV<Integer, Iterable<T>>, WriteT> {
        ...
        public void processElement(ProcessContext c, BoundedWindow window) throws Exception {
            ...
            if (windowedWrites) {
                writer.openWindowed(UUID.randomUUID().toString(), window, c.pane(), c.element().getKey(), numShards);
            } else {
                writer.openUnwindowed(UUID.randomUUID().toString(), UNKNOWN_SHARDNUM, UNKNOWN_NUMSHARDS);
            }
            LOG.debug("Done opening writer {} for operation {}", writer, writeOperationView);
    ...
    private static class ApplyShardingKey<T> extends DoFn<T, KV<Integer, T>> {
        ...
        ApplyShardingKey(PCollectionView<Integer> numShardsView, ValueProvider<Integer> numShardsProvider) {
            this.numShardsView = numShardsView;
            this.numShardsProvider = numShardsProvider;
        ...
        public void processElement(ProcessContext context) {
            ...
            checkArgument(shardCount > 0,
                    "Must have a positive number of shards specified for non-runner-determined sharding." + " Got %s",
                    shardCount);
     ...
     *
     * <p>
     * In the first, a do-once ParDo is applied to a singleton PCollection containing the Sink's {@link WriteOperation}.
     * In this initialization ParDo, {@link WriteOperation#initialize} is called. The output of this ParDo is a
     * singleton PCollection containing the WriteOperation.
     *
     * <p>
     * This singleton collection containing the WriteOperation is then used as a side input to a ParDo over the
     * PCollection of elements to write. In this bundle-writing phase, {@link WriteOperation#createWriter} is called to
     * obtain a {@link Writer}. {@link Writer#open} and {@link Writer#close} are called in {@link DoFn#startBundle} and
     * {@link DoFn#finishBundle}, respectively, and {@link Writer#write} method is called for every element in the
     * bundle. The output of this ParDo is a PCollection of <i>writer result</i> objects (see {@link Sink} for a
     * description of writer results)-one for each bundle.
     *
     * <p>
     * The final do-once ParDo uses the singleton collection of the WriteOperation as input and the collection of writer
     * results as a side-input. In this ParDo, {@link WriteOperation#finalize} is called to finalize the write.
     *
     * <p>
     * If the write of any element in the PCollection fails, {@link Writer#close} will be called before the exception
     * that caused the write to fail is propagated and the write result will be discarded.
     *
     * <p>
     * Since the {@link WriteOperation} is serialized after the initialization ParDo and deserialized in the
     * bundle-writing and finalization phases, any state change to the WriteOperation object that occurs during
     * initialization is visible in the latter phases. However, the WriteOperation is not serialized after the
     * bundle-writing phase. This is why implementations should guarantee that {@link WriteOperation#createWriter} does
     * not mutate WriteOperation).
     */
    private <WriteT> PDone createWrite(PCollection<T> input, WriteOperation<T, WriteT> writeOperation) {
        Pipeline p = input.getPipeline();
        writeOperation.setWindowedWrites(windowedWrites);

        // A coder to use for the WriteOperation.
        @SuppressWarnings("unchecked")
        Coder<WriteOperation<T, WriteT>> operationCoder = (Coder<WriteOperation<T, WriteT>>) SerializableCoder
                .of(writeOperation.getClass());

        // A singleton collection of the WriteOperation, to be used as input to a ParDo to initialize
        // the sink.
        PCollection<WriteOperation<T, WriteT>> operationCollection = p.apply("CreateOperationCollection",
                Create.of(writeOperation).withCoder(operationCoder));

        // Initialize the resource in a do-once ParDo on the WriteOperation.
        operationCollection = operationCollection
                .apply("Initialize", ParDo.of(new DoFn<WriteOperation<T, WriteT>, WriteOperation<T, WriteT>>() {
        ...
        final PCollectionView<WriteOperation<T, WriteT>> writeOperationView = operationCollection
                .apply(View.<WriteOperation<T, WriteT>> asSingleton());
        ...
        if (computeNumShards == null && numShardsProvider == null) {
            if (windowedWrites) {
                throw new IllegalStateException(
                        "When doing windowed writes, numShards must be set" + "explicitly to a positive value");
            }
            numShardsView = null;
            results = input.apply("WriteBundles",
                    ParDo.of(new WriteBundles<>(writeOperationView)).withSideInputs(writeOperationView));
        } else {
            if (computeNumShards != null) {
                numShardsView = input.apply(computeNumShards);
                results = input
                        .apply("ApplyShardLabel",
                                ParDo.of(new ApplyShardingKey<T>(numShardsView, null)).withSideInputs(numShardsView))
                        .apply("GroupIntoShards", GroupByKey.<Integer, T> create())
                        .apply("WriteShardedBundles", ParDo.of(new WriteShardedBundles<>(writeOperationView, numShardsView))
                                .withSideInputs(numShardsView, writeOperationView));
            } else {
                numShardsView = null;
                results = input.apply("ApplyShardLabel", ParDo.of(new ApplyShardingKey<T>(null, numShardsProvider)))
                        .apply("GroupIntoShards", GroupByKey.<Integer, T> create()).apply("WriteShardedBundles",
                                ParDo.of(new WriteShardedBundles<>(writeOperationView, null)).withSideInputs(writeOperationView));
            }
        }
        ...
        if (windowedWrites) {
            ...
            PCollection<KV<Void, WriteT>> keyedResults = results.apply("AttachSingletonKey",
                    WithKeys.<Void, WriteT> of((Void) null));
            keyedResults.setCoder(KvCoder.<Void, WriteT> of(VoidCoder.of(), writeOperation.getWriterResultCoder()));

            // Is the continuation trigger sufficient?
            keyedResults.apply("FinalizeGroupByKey", GroupByKey.<Void, WriteT> create()).apply("Finalize",
                    ParDo.of(new DoFn<KV<Void, Iterable<WriteT>>, Integer>() {
            ...
            final PCollectionView<Iterable<WriteT>> resultsView = results.apply(View.<WriteT> asIterable());
            ...
            operationCollection.apply("Finalize", ParDo.of(new DoFn<WriteOperation<T, WriteT>, Integer>() {
                ...
                public void processElement(ProcessContext c) throws Exception {
                    ...
                    if (extraShardsNeeded > 0) {
                        LOG.info("Creating {} empty output shards in addition to {} written for a total of " + " {}.",
